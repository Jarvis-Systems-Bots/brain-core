...
class BrainDocsCommand extends IncludeArchetype
{
...
---')->key('structure')
        ...
        $this->guideline('documentation-philosophy')
            ->text('Golden rules: Documentation is for HUMANS. Clarity over completeness. Description over code.')
            ->example('Primary: Clear textual explanation of concepts, workflows, architecture')->key('text-first')
            ->example('Secondary: Small, essential code snippets only when text insufficient')->key('code-minimal')
            ->example('Forbidden: Large code blocks, full implementations, copy-paste from codebase')->key('no-code-dumps')
            ->example('Goal: Human understanding, not code reference')->key('human-first');
