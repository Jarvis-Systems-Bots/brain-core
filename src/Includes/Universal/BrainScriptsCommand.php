...
class BrainScriptsCommand extends IncludeArchetype
{
...
class ExampleScript extends Command
{
    ...
    public function handle(): void
    {
        ...
        $this->guideline('scope-visibility-critical')
            ->text('CRITICAL: Scripts execute in Brain context, completely isolated from project code.')
            ->example('Scripts run in Brain ecosystem (BrainScripts namespace)')->key('context')
            ->example('Project classes/code NOT visible - scripts are Brain tools, not project code')->key('isolation')
            ...
            ->example('Scripts interact with project via external interfaces only')->key('philosophy');
        ...
        $this->guideline('workflow-project-integration')
            ->text('How scripts interact with project code (scripts are isolated from project).')
